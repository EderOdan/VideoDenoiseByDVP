...
# 	# dense_flow(tenOutput)
# 	# backwarp
# 	tenOutput = estimate(tenFirst, tenSecond).permute(0,2,3,1).numpy() #(1,436,1024,2)
# 	tenSecond = tenSecond.unsqueeze(0).permute(0,2,3,1).numpy()*255
# 	print(tenOutput)
# 	print(tenOutput.shape)
# 	print(tenFirst.shape)
# 	deformed_nearest = image_warp(tenSecond.copy(), tenOutput, mode='bilinear')
# 	out_img = deformed_nearest[0]
# 	print(out_img.shape)
...
# 	cv2.imwrite('./test.png', out_img)
	...
	# tenPreprocessedFirst = tenFirst.cuda().view(1, 3, intHeight, intWidth)
	# tenPreprocessedSecond = tenSecond.cuda().view(1, 3, intHeight, intWidth)
	# # print(backwarp_tenGrid)
	#
	# esti_pic = backwarp(tenPreprocessedFirst, tenOutput).cpu() * 255
	# # esti_pic = warp(tenPreprocessedFirst,tenOutput).cpu()* 255
	# # esti_pic = warp_pic.cpu() * 255
	# out_img = esti_pic.clamp(0, 255).type(torch.uint8)[
	# 	0].permute(1, 2, 0).numpy()
	# # out_img = cv2.cvtColor(out_img, cv2.COLOR_RGB2BGR)
	#
	# print(out_img.shape)
	# cv2.imwrite('./test.png', out_img)
	# print(esti_pic.shape)
